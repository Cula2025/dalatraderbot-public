...
# ---------- Hjälp: YoY & tradestatistik --------------------------------------

def yoy_stats(eq: pd.DataFrame) -> Dict[str, float]:
    """Year-over-Year statistik på equity-kurvan."""
    if eq.empty:
...
# ---------- Signaler (inbyggda här för att minska beroenden) -----------------
...
def build_signals(df: pd.DataFrame, strategy: str,
    ...
    if strategy == "breakout":
        ...
        raise ValueError("strategy måste vara 'breakout' eller 'sma_cross'")

# ---------- Griddef -----------------------------------------------------------

def grid_params(mode: str = "medium") -> Dict[str, List]:
    """
    Diskreta värdelistor för parametrar.
    'small' ~ snabb, 'medium' ~ rimlig, 'wide' ~ bred (kan ta längre tid).
    """
    if mode == "small":
        return dict(
            strategy=["breakout"],
...
def main():
    ap = argparse.ArgumentParser(description="Grid-tuner för Börsdata, inkl. Y/Y-analys")
    ...
    ap.add_argument("--out", default=None, help="CSV-sökväg (default: outputs/grid_<TICKER>_<MODE>.csv)")
    ap.add_argument("--max-combos", type=int, default=None, help="(valfritt) slumpmässigt tak på antal kombos")
    args = ap.parse_args()

    # Hämta data
    print(f"[INFO] Hämtar {args.ticker} från Börsdata…")
    df = load_ohlcv(args.ticker, days=20000, start=args.start, end=args.end)
    if df.shape[0] < 60:
        raise RuntimeError(f"För få rader ({len(df)}) efter filtrering.")
    ...
    for i, values in enumerate(combos, 1):
        ...
        if i % 50 == 0:
            print(f"[INFO] Kört {i}/{len(combos)} …")
    ...
    # Sortera: bästa först (hög Sharpe, mindre negativ MaxDD, hög CAGR)
